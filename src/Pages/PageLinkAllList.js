    ...
    <Container>
      <BasicLayout>
        ...
        <PageSearch onSubmit={handleSubmit(onSubmit)}>
          <div></div>
          <Input {...register("search", { required: true })} placeholder="추천 페이지 제목을 입력하세요." autoComplete="off" />
            ...
            {data?.seePageLink.map((item, index) => {
              return (
                <Item key={index}>
                  ...
                  <LinkIcon onClick={() => onClickLink(item.pageURL)}>
                    <FiLink />
                  </LinkIcon>
                  <EditIcon onClick={() => onCLickEdit(item.pageTitle)}>
                    <BiEdit />
                  </EditIcon>
                </Item>
              );
