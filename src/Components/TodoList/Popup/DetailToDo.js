...
import {
  COMPLETE_TO_DO_LIST_MUTATION,
  DELETE_TO_DO_LIST_MUTATION,
  EDIT_TO_DO_LIST_MUTATION,
} from "../../../Graphql/ToDoList/mutation";
...
const DetailToDo = ({ setMsg, setErrMsg, userEmail, setRefetchQuery }) => {
  ...
  return (
    <PopupContainer maxHeight={true} needAlert={type !== "complete" && true}>
      <PopupForm create={false} onSubmit={handleSubmit(onSubmit)} type={type}>
        ...
        {type !== "complete" && <SubmitBtn type="submit" value="수정하기" />}
        <BottomBtn type={type}>
          {type !== "complete" && (
            <CompleteBtn onClick={onClickCompleteBtn} className="bottom_btn">
              완료하기
            </CompleteBtn>
          )}
          <DelBtn onClick={onClickDelBtn} className="bottom_btn">
            삭제하기
          </DelBtn>
        </BottomBtn>
      </PopupForm>
