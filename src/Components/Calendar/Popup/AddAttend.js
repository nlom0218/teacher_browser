...
const AddAttend = ({ userEmail, setErrMsg, setMsg, setRefetchQuery, urlDate }) => {
  ...
  return (<PopupContainer maxHeight={true}>
    <CalenderPopupFormContainer onSubmit={handleSubmit(onSubmit)}>
      <CalenderPopupTitle>출석등록</CalenderPopupTitle>
      <InputLayout>
      ...
      <InputLayout>
        ...
        <AttendType>
          <Type onClick={() => setType(1)} selected={type === 1}>인정 결석</Type>
          <Type onClick={() => setType(2)} selected={type === 2}>질병 결석</Type>
          <Type onClick={() => setType(3)} selected={type === 3}>미인정 결석</Type>
          <Type onClick={() => setType(4)} selected={type === 4}>기타 결석</Type>
          <Type onClick={() => setType(5)} selected={type === 5}>지각</Type>
          <Type onClick={() => setType(6)} selected={type === 6}>조퇴</Type>
          <Type onClick={() => setType(7)} selected={type === 7}>결과</Type>
        </AttendType>
      </InputLayout>
