...
const CalendarItem = ({
  ...
  const {
    ...
    loading: toDoLoading,
    refetch,
  } = useQuery(SEE_TO_DO_LIST_ONLY_LENGTH_QUERY, {
    variables: {
      userEmail,
  ...
  return (
    <Container>
      <Day sun={getDay(item.date) === 0} curMonth={item.month === "cur"}>
        <Date isToday={isToday(item.date)} onClick={onClickDay}>
          {getDate(item.date)}
          {media !== "Mobile" && "일"}
        </Date>
      </Day>
            ...
            <React.Fragment>
              ...
              <ScheduleList row={row}>
                {dateSchedule.length !== 0 &&
                  dateSchedule?.map((scheduleItem, index) => {
                    if (!scheduleItem) {
                      return (
              ...
              <Summary>
                {!toDoLength?.seeToDoListOnlyLength ? (
                  <div></div>
                ) : (
                  toDoLength?.seeToDoListOnlyLength !== 0 && (
                    <ToDoLength>
                      <ToDoIcon>
                        <IcToDoList />
                      </ToDoIcon>
